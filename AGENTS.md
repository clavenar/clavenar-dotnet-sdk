...
- **Duck-typing shape guard.** `InspectResponseAsync` duck-types an Anthropic message (`content[]` with `type:"tool_use"`) or an OpenAI completion (`choices[].message.tool_calls[]`, filtered to `type:"function"`) via JSON, then forwards the extracted calls to `InspectAllAsync`. A response that yields *zero* extracted calls is a silent no-op — `InspectAllAsync` early-returns on `calls.Count == 0` — which is the correct outcome for a text-only turn (no `tool_use` / `tool_calls` to gate). Don't add a throw here; a model turn with no tool calls is normal, not a contract error.
